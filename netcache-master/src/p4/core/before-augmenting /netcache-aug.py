...
def create_edges(filename):
    ...
    for key in table_next_actions.keys():
        ...
        for k,v in zip(keys, vals):
            if len_val==2 and k!=v and k!='NoAction':
    ...
    edges.append({'src':"NoAction", 'dst':"node_49", 'weight':0})
...
def create_cfg(filename):
    ...
    G.remove_edge("MyIngress.debug","NoAction")


    print("number of edges are",len(G.edges))
    ...
    print("leaf vertexes are:",)
    for i in range(len(leaf_vertex)):
    	print("leaf number " + str(i) +" is " + str(leaf_vertex[i]))
...
def augmentor(p4_filename, json_filename):
    ...
    print("number of possible paths are", len(path_list))
