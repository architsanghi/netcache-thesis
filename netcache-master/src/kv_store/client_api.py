...
# prev_latency= 0.1
temp_time= time.time()
attack_time= open("attack-time-ground-truth.txt",'a')
file_attack_info= open("../p4/images/attack-info.txt",'r')
...
start_query_number = int(float(required_array[0])*total_queries)
end_query_number =  int(float(required_array[1])*total_queries)
current_query= 0
...
class NetCacheClient:
    ...
    def read(self, key, seq=0, suppress=False):
        ...
        global temp_time
        global start_query_number
        global end_query_number
        ...
        if current_query == start_query_number:
            time_elapsed= time.time() - temp_time
            attack_time.write(str(time_elapsed))
            attack_time.write(" ")
        if current_query == end_query_number:
            last_time= time.time()- temp_time
            attack_time.write(str(last_time))
            attack_time.write("\n")
